...
# MAGIC %sh
# MAGIC rm -rf "/dbfs/toxicity_accelerator/_checkpoints/Toxicity_Chat_Pred"
...
comments_pred.writeStream\
  .trigger(once=True)\
  .format("Delta")\
  .option("checkpointLocation", "/toxicity_accelerator/_checkpoints/Toxicity_Chat_Pred")\
  .table("Gaming.Toxicity_Chat_Pred")
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS Gaming.Toxicity_Chat_Pred

# COMMAND ----------

chatDF = spark.table("Gaming.Toxicity_Chat").withColumnRenamed('key', 'comment_text')
chatDF = model.transform(chatDF)\
  .withColumn('predicted',col('class.result'))\
  .drop('document', 'token', 'universal_embeddings', 'class')

chatDF.write.format("delta").saveAsTable("Gaming.Toxicity_Chat_Pred", overwrite=True)
...
# MAGIC %sql
# MAGIC SELECT match_id, comment_text, slot, time, unit, predicted FROM Gaming.Toxicity_Chat_Pred LIMIT 1
...
# MAGIC FROM Gaming.Toxicity_chat_pred
...
# MAGIC %sql
# MAGIC SELECT 'Toxic', count(*) AS Message_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic')
# MAGIC UNION
# MAGIC SELECT 'Non-Toxic', count(*) FROM Gaming.Toxicity_chat_pred WHERE size(predicted) > 0
...
# MAGIC   SELECT 'Toxic', count(*) AS Message_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic')
# MAGIC   UNION
# MAGIC   SELECT 'Obscene', count(*) AS Message_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'obscene')
# MAGIC   UNION
# MAGIC   SELECT 'Insult', count(*) AS Message_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'insult')
# MAGIC   UNION
# MAGIC   SELECT 'Threat', count(*) AS Message_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'threat')
# MAGIC   UNION
# MAGIC   SELECT 'Identity_hate', count(*) AS Message_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'identity_hate')
# MAGIC   UNION
# MAGIC   SELECT 'Severe_toxic', count(*) AS Message_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'severe_toxic')
# MAGIC )
...
# MAGIC %sql
# MAGIC SELECT size(predicted) AS Number_of_Labels, count(*) AS Message_Count FROM Gaming.Toxicity_chat_pred WHERE size(predicted) > 0 GROUP BY size(predicted) ORDER BY size(predicted) ASC
...
# MAGIC %sql
# MAGIC SELECT Toxic AS Label_Type, Match_Count, Round((Match_Count/(SELECT count(DISTINCT match_id) AS Match_Count FROM Gaming.Toxicity_chat_pred))*100) AS Percent_of_total_matches from (
# MAGIC   SELECT 'Toxic', count(DISTINCT match_id) AS Match_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic')
# MAGIC   UNION
# MAGIC   SELECT 'Obscene', count(DISTINCT match_id) AS Match_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'obscene')
# MAGIC   UNION
# MAGIC   SELECT 'Insult', count(DISTINCT match_id) AS Match_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'insult')
# MAGIC   UNION
# MAGIC   SELECT 'Threat', count(DISTINCT match_id) AS Match_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'threat')
# MAGIC   UNION
# MAGIC   SELECT 'Identity_hate', count(DISTINCT match_id) AS Match_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'identity_hate')
# MAGIC   UNION
# MAGIC   SELECT 'Severe_toxic', count(DISTINCT match_id) AS Match_Count FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'severe_toxic')
# MAGIC )
...
# MAGIC   FROM (
# MAGIC     SELECT 'Game_Lobby', count(comment_text) `Number_of_toxic_messages` FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic') AND time < 0
# MAGIC     UNION 
# MAGIC     SELECT 'Less_Than_5_minutes', count(comment_text) `Number_of_toxic_messages` FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic') AND time < 300
# MAGIC     UNION 
# MAGIC     SELECT '5-10_minutes', count(comment_text) `Number_of_toxic_messages` FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic') AND time BETWEEN 300 AND  600
# MAGIC     UNION
# MAGIC     SELECT '10-20_minutes', count(comment_text) `Number_of_toxic_messages` FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic') AND time BETWEEN 600 AND  1200
# MAGIC     UNION
# MAGIC     SELECT '20-30_minutes', count(comment_text) `Number_of_toxic_messages` FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic') AND time BETWEEN 1200 AND 1800
# MAGIC     UNION
# MAGIC     SELECT '30-40_minutes', count(comment_text) `Number_of_toxic_messages` FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic') AND time BETWEEN 1800 AND 2400
# MAGIC     UNION
# MAGIC     SELECT '40+minutes', count(comment_text) `Number_of_toxic_messages` FROM Gaming.Toxicity_chat_pred WHERE array_contains(Toxicity_chat_pred.predicted, 'toxic') AND time > 2400
# MAGIC )
